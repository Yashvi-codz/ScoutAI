...
export default function CompareAthletes() {
  ...
  return (
    <div className="page-pad">
      ...
      {/* Selectors */}
      <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 20, alignItems: 'center', marginBottom: 28, animationDelay: '0.06s' }}>
        <div>
        ...
        <div style={{ fontFamily: 'var(--font-head)', fontSize: 24, color: 'var(--muted)', letterSpacing: 2, textAlign: 'center' }}>VS</div>
        <div>
      ...
      {/* Hero comparison */}
      <div className="fade-up" style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 20, marginBottom: 24, animationDelay: '0.10s' }}>
        {[left, right].map((ath, idx) => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{
            ...
            fontFamily: 'var(--font-head)', fontSize: 14, color: 'var(--muted)',
          }}>VS</div>
        </div>
      </div>
      ...
      <div className="card fade-up" style={{ animationDelay: '0.18s' }}>
        <SectionHeader title="METRIC COMPARISON" subtitle="Detailed head-to-head breakdown" />
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ textAlign: 'right', padding: '8px 12px', fontSize: 10, color: 'var(--muted)', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', width: '30%', borderBottom: '1px solid var(--border)' }}>{left.name.split(' ')[0]}</th>
              <th style={{ textAlign: 'center', padding: '8px 12px', fontSize: 10, color: 'var(--muted)', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', borderBottom: '1px solid var(--border)' }}>METRIC</th>
              <th style={{ textAlign: 'left', padding: '8px 12px', fontSize: 10, color: 'var(--muted)', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', width: '30%', borderBottom: '1px solid var(--border)' }}>{right.name.split(' ')[0]}</th>
            </tr>
          </thead>
          <tbody>
            {metrics.map((m) => {
                ...
                <tr key={m} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '14px 12px', textAlign: 'right' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10 }}>
                      <div style={{ flex: 1, height: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden', direction: 'rtl' }}>
